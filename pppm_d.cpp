...
PPPMD::PPPMD(LAMMPS *lmp, int narg, char **arg) : KSpace(lmp, narg, arg)
{
...
//  distance = (double) sqrt(powf((double)k,2));
  lambda   = (double) (order/2);
  Re       = (double) (order/4);
...
void PPPMD::compute(int eflag, int vflag)
{
  int i,j;
...
double PPPMD::compute_df_kspace()
{
  double xprd = domain->xprd;
  double yprd = domain->yprd;
  double zprd = domain->zprd;
...
double PPPMD::estimate_ik_error(double h, double prd, bigint natoms)
{
  double sum = 0.0;
...
void PPPMD::set_grid_local()
{
 ...
 //  else shiftone = 2;
  ...
  //   qd/ist = offset due to TIP4P fictitious charge
...
void PPPMD::compute_gf_ik_triclinic()
{
  double snx,sny,snz;
  double argx,argy,argz,wx,wy,wz,sx,sy,sz,qx,qy,qz;
  double sum1,dot1,dot2;
...
void PPPMD::compute_sf_precoeff()
{
  int i,k,l,m,n;
...
//   nx = static_cast<int> (x[i][0] - 0.3);
//   ny = static_cast<int> (x[i][1] - 0.3);
//   nz = static_cast<int> (x[i][2] - 0.3);
...
void PPPMD::make_rho()
{
  int l,k,m,p,n,num_nodes,nx,ny,nz,mx,my,mz;
...
void PPPMD::poisson_ik_triclinic()
{
  int i,j,k,n;
...
void PPPMD::fieldforce_ad()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    ny = part2grid[i][1];
    nz = part2grid[i][2];
//    dx = 0.2+nx+shiftone - (x[i][0]-boxlo[0])*delxinv;
//    dy = 0.2+ny+shiftone - (x[i][1]-boxlo[1])*delyinv;
//    dz = 0.2+nz+shiftone - (x[i][2]-boxlo[2])*delzinv;
...
void PPPMD::pack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  int n = 0;
...
void PPPMD::unpack_forward(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  int n = 0;
...
void PPPMD::pack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
...
void PPPMD::unpack_reverse(int flag, FFT_SCALAR *buf, int nlist, int *list)
{
  if (flag == REVERSE_RHO) {
...
void PPPMD::procs2grid2d(int nprocs, int nx, int ny, int *px, int *py)
{
...
void PPPMD::compute_rho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                         const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
...
//  double PI = 3.1428;
  ...
  for (k = (1-order)/2; k <= order/2; k++) {
    ...
    distance = sqrt(pow((double)k,2));
    factor1  = pow(distance-Re,2)/lambda;
...
void PPPMD::compute_drho1d(const FFT_SCALAR &dx, const FFT_SCALAR &dy,
                          const FFT_SCALAR &dz)
{
  int k,l;
  FFT_SCALAR r1,r2,r3;
  ...
  for (j = 1; j < order; j++) {
    for (k = -j; k <= j; k += 2) {
      ...
      for (l = 0; l < j; l++) {
        ...
        s += powf(0.5,(double) l+1) *
          (a[l][k-1] + powf(-1.0,(double) l) * a[l][k+1]) / (l+1);
...
int PPPMD::timing_1d(int n, double &time1d)
{
  double time1,time2;
...
int PPPMD::timing_3d(int n, double &time3d)
{
  double time1,time2;
...
void PPPMD::compute_group_group(int groupbit_A, int groupbit_B, int BA_flag)
{
  if (slabflag)
...
void PPPMD::make_rho_groups(int groupbit_A, int groupbit_B, int BA_flag)
{
...
//    dz = 0.001;
   ...
   //             density_A_brick[mz][my][mx] = q[i]*rho1d[0][l];
            // group B

            if (mask[i] & groupbit_B)
              density_B_brick[mz][my][mx] += x0*rho1d[0][l];
  //              density_B_brick[mz][my][mx] = q[i]*rho1d[0][l];
          }
        }
      }
    }
...
void PPPMD::poisson_groups(int BA_flag)
{
  int i,j,k,n;
...
void PPPMD::poisson_groups_triclinic()
{
  int i,j,k,n;
